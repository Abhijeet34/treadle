...
/**
 * Everything that decides a corpus's bytes: the spec, and the source of the two things that
 * write it. A cache entry is named by this, so a corpus generated by an older generator or
 * against an older store grammar can never be found at the path a newer run looks up. That is
 * what makes reuse safe without a validation pass over hundreds of megabytes of records.
 */
