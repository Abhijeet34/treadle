...
//
// `bin/treadling.js` carries the shebang and the reasoning behind it; the build reads it from
...
/** The first line of `bin/treadling.js`, which is the shebang every executable of this tool gets. */
export function shebangOf(root: string): string {
  return readFileSync(path.join(root, 'bin', 'treadling.js'), 'utf8').split('\n')[0] as string
}
