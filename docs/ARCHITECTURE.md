...
This file is the map; [DOMAIN.md](DOMAIN.md) is the detail for the only layer that exists today.
...
Today `src/application`, `src/adapters` and `src/cli` hold a README each and no code.
That is deliberate: the domain core landed first because every other decision is exercised through it.
...
|---|---|---|---|
| Store | Reads records and events by id, state, sprint and time range; applies a transaction under a lock with compare-and-set | Sharded Markdown files with a SQLite index | An overlay store: a copy-on-write layer over a base store, which is how `--dry-run` and `--preview` evaluate every guard without writing |
...
The Policy seam is the one that already exists.
