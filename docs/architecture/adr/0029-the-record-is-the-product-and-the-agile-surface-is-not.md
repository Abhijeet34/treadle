...
That is the file-format promise in docs/STABILITY.md kept by the mechanism that was already there: no workspace breaks and no migration is required.
But the retired key does not go away either: this build has no way to clear a field from stored data once it has been written, so it rides `extra` forever, on every record that carried it before the cut.
That is a gap, not a design. Any workspace that outlives a field removal inherits the same dead keys for as long as the record exists, and clearing them is queued work this cut does not do.
treadle's own `.work` store keeps `points` on every pre-cut item for that reason and no other: the tool cannot clear a retired key, and a hand edit of the shard would remove the field with no actor and no event, which is exactly what this project holds every other change to a record to. The project holds itself to the same standard, and will remove that clutter through `treadle` itself, with an actor and an event, once the capability exists.
