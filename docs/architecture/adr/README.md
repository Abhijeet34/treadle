...
| [ADR-0035](0035-a-verdict-that-records-nothing-and-a-rendering-for-a-person-go.md) | The `gate` command and the Markdown export are cut because a verdict that records nothing and a rendering for a person move no load into a file that survives a crash; threat-model finding F4 closes by absence with the export it waited on |
| [ADR-0036](0036-eperm-from-an-exclusive-create-is-contention-when-the-directory-takes-a-file.md) | An exclusive create refused with `EPERM` is contention when the directory still takes a file and `S11` when it does not, because Windows answers a delete-pending lock file with the errno POSIX reserves for a permission failure |
| [ADR-0037](0037-the-automation-cuts-the-tag-and-no-release-carries-a-human-signature.md) | release-please creates the release tag behind a three-platform matrix and the release pull request's parked runs approve themselves, so merging that pull request is the only human act; the signature clause it replaces had never been able to pass on a runner, and no release carries a human signature any more |
| [ADR-0038](0038-the-name-is-treadling-and-the-old-npm-record-belongs-to-another-developer.md) | The package, the command, the `TREADLING_*` environment variables and the repository are named `treadling`, because another developer holds the npm ownership record for `treadle` for a real unrelated library and npm transfers no name on demand; the records that measured the old name keep it |
| [ADR-0039](0039-the-published-name-is-scoped-and-the-similarity-gate-is-only-observable-on-a-publish.md) | The published package name is `@abhijeet34/treadling`, because npm refused the unscoped `treadling` as too similar to `readline` at two edits and the clearance screen tested one; the typed command, the `TREADLING_*` environment variables and the repository keep the bare word |
