...
**Overtaken in part by:** [ADR-0038](0038-the-name-is-treadling-and-the-old-npm-record-belongs-to-another-developer.md), which answers the identity gate this record left open: the sixteen-source clearance screen has run, and it cleared `treadling` rather than the name spelled throughout below. So `treadle` here is the old name of this package, its command, its `TREADLE_*` environment variables and its repository, and the paths `dist/treadle.js` and `bin/treadle.js` are now `dist/treadling.js` and `bin/treadling.js`; the bundle as the published product, the three publication interlocks, F13's three controls and every alternative measured here stand unchanged.
