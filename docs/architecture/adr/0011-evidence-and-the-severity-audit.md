...
**Status:** Accepted
**Date:** 2026-09-05
**Implements:** sections 3.4 and 4.5 of the `treadling-board-outcomes-retention-b3` audit
...
**Overtaken in part by:** [ADR-0034](0034-the-reviewer-guard-stops-policing-and-starts-recording.md) removes `DOD3`'s actor half, so the anti-attestation pair below no longer reads that a story reaches `done` only when someone other than its maker accepted it; what the pair holds now is that the record names a reviewer other than its assignee and points at something a third party can open, and `DOD7`, the evidence bounds and `H21` stand
