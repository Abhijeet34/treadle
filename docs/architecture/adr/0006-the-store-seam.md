...
**Overtaken in part by:** [ADR-0017](0017-an-impediment-is-a-type-that-blocks.md), which gave one of the absent entities its type, and the doctor this record deferred is built
**Overtaken in part by:** [ADR-0029](0029-the-record-is-the-product-and-the-agile-surface-is-not.md), which leaves the overlay to back `--dry-run` alone
**Overtaken in part by:** [ADR-0030](0030-the-index-goes-and-a-read-parses-the-files.md), which removes the SQLite index `ShardedStore` read and wrote through; it reads the shards directly, and `eachItem`/`eachEvent` iterate a parse
