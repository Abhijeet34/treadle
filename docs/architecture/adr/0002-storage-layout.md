...
**Overtaken in part by:** [ADR-0017](0017-an-impediment-is-a-type-that-blocks.md), which settles that `impediments.md` is never written
**Overtaken in part by:** [ADR-0029](0029-the-record-is-the-product-and-the-agile-surface-is-not.md), which leaves the month-sharded items and the log as the whole layout
**Overtaken in part by:** [ADR-0030](0030-the-index-goes-and-a-read-parses-the-files.md), which removes the derived index this record put in front of the shards; the shard, the log, the atomic write and the freshness stat stand, and the stat now decides whether one command's parse is still its answer
