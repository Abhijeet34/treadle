...
| 12 processes each transitioning a distinct item through `bin/treadling.js`, released together on a workspace whose index was deleted | 1 crash and 1 write not applied in 120 transitions with the busy timeout armed second, 0 in 240 with it armed first |
...
Every temp file under the root, `items/` and `.txn/` is removed by the next lock holder, whatever its age.
Only a lock holder writes one, so a temp file standing when the lock changes hands belongs to a holder that never committed: a writer killed mid-write, or a descheduled one still carrying a rename.
Removing it is what makes that rename fail rather than land, which is the reclaim fence the next section ends on.
...
- **A duplicated id refuses writes to that id, not every write in the workspace.** The domain model's H15 puts the refusal in `doctor` and stops the workspace until the duplicate is fixed. `doctor` was not built when this was written, and stopping unrelated work is a larger hammer than the ambiguity needs: a write to any other id names exactly one record and leaves both copies of the duplicated one untouched. The wider refusal lands with `doctor`, which is the command H15 actually names.
  That is still what the store does and is no longer what a caller sees. ADR-0003 rule 7 moved the finding check into `readWorkspace`, the one read every command is built on, so a finding that hides a record the store holds refuses the view: with two records sharing an id, `backlog` and a `file` of an unrelated id both exit 7 naming `S3`, its two lines and `treadling doctor` as the fix. The domain model's H15 got its hammer after all, at the layer that could see the whole set rather than at the one that could see one write.
- **`--lock-timeout` is a store option rather than a flag.** The flag belongs to the CLI, which was not built when this was written. The option it maps to is here and is tested, and the CLI that arrived in #3 exposes no such flag: `docs/STABILITY.md` makes adding one a non-breaking change whenever a caller needs it.
- **A holder knows when its lock was reclaimed, and refuses to write past that point.** DR4 covers the waiter's half of a stalled heartbeat and not the holder's: a holder paused past the 5 second window resumes believing it still holds the lock, and wrote a shard from the parse it took before the pause over the write the reclaimer had landed. Measured with `SIGSTOP` on a writer inside its critical section, which is what `Ctrl-Z` in a shell does: one lost update in 117 writes, `doctor` clean. The handle now records the gap between heartbeats and the token on disk, and the store asks it at the commit point of every file it writes, after the fsync and before the rename, where the window is one syscall wide; a lost lock is refusal `S16`, saying whether the journal was written and so lands through the next holder. A check cannot close the window it opens, and this one did not: a holder descheduled between the answer and the rename still committed, which reads on disk as a version one below the update events that landed. CI caught it once in three runs on Node 26 and thirty local runs never did. The fence is the temp file rather than the check. A holder that has just taken the lock removes every temp file standing under the root, `items/` and `.txn/`, so a resumed writer renames from a source that is gone, is asked the lock again on that failure, and refuses with `S16` instead of landing. The ordering is the whole argument: a temp file the sweep removed was created before the sweep and so before this holder read anything, and a rename that beat the sweep landed before that read took the files. `test/store/reclaim-fence.test.ts` holds the window open from inside the guard rather than waiting for a scheduler to produce it, and it loses a write 20 times in 20 against the pre-fix tree and none in 20 after. The refresh that used to run cold under the lock now runs once before it as well, because a 1.1 million event rebuild is two minutes of synchronous work during which no heartbeat fires and the lock is forfeit by the rule above. Sweeping the root along with `items/` and `.txn/` reaches one file the fence does not own: `init` writes `.gitignore` and `.gitattributes` by temp-and-rename before the workspace has a lock to take. A transaction reclaiming a lock on that still-being-created root sweeps the temp file out from under `init`, whose own rename then fails and refuses `STORE_UNAVAILABLE` instead of finishing. No write a transaction commits is any less safe for it, and no earlier state protected `init` from this either, so the exposure is accepted rather than closed.
