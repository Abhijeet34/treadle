...
4. **A body line beginning with `#` at column 0 is escaped, not refused.** Such a line would re-parse as a heading, so the canonical form writes one backslash in front of it and the parser takes one off: `## Findings` is stored as `\## Findings`. That is CommonMark's own escape, so a markdown reader still renders the line as the text the record holds, and it is a bijection over exactly those lines, so prose that already reads `\## x` is stored as `\\## x` and comes back unchanged. What the write path does refuse, by name, is a body that would be read back as a second record: a line rule 1 resynchronises on, with a mandatory field block under it. It measures that with rule 1's own predicate rather than a second one, so the write bound and the read cannot disagree. On read, a hand-edited body line that opens a record is still quarantined by the segment split rather than silently re-homed.
...
- A section body line reading `some-slug: some text` at column 0, directly above what looks like a record's mandatory field block, is read as a damaged heading and the record is quarantined. That shape is what a swallowed record looks like, so the false positive and the true positive are the same bytes; the refusal names the record and the line. No write produces it any more, because rule 4 refuses it at the write with the same predicate, so it reaches a file only by a hand edit or a merge.
...
- **Rule 4 escapes a heading rather than refusing it, and its bound is rule 1's own predicate.** The fourth correction to this record, and it closes two defects that turned out to be one.
  The first shipped version refused any body line beginning with `#` and bounded nothing else.
