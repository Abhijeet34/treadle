...
Two rows left this table with ADR-0029 rather than being re-measured: "an item is in one sprint, and a close records what it did not finish" and "a sprint record does not move the read every command performs". Both were true of the tree that carried the sprint, and neither is a claim about this one.
