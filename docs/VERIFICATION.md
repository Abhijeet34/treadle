...
Three things were not run and none of them is proven by any of that: `actions/attest-build-provenance`, which needs the workflow's OIDC identity; `gh release create --verify-tag`, which needs the tag to exist; and the `publish` and `smoke` jobs, which need a publish.
